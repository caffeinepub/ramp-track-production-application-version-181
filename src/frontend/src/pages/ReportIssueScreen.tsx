...
export default function ReportIssueScreen({ onBack }: ReportIssueScreenProps) {
  ...
  const [step, setStep] = useState<"input" | "details" | "photo" | "success">(
    "input",
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log(
        "[ReportIssueScreen] Issue reporting uses auth as identity source",
      );
  ...
  if (showScanner && scannerMountedRef.current) {
    return (
      <EquipmentQRScanner
        mode="equipment"
        title="Scan Equipment to Report Issue"
        onScan={handleScan}
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex gap-2 pt-4">
                    ...
                    <Button
                      ...
                      disabled={reportIssue.isPending || isValidatingSession}
                      className="flex-1"
                    >
                      {isValidatingSession ? (
                        <>
