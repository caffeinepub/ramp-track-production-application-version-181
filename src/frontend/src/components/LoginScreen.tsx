...
export default function LoginScreen({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(
        "[LoginScreen] Manual login successful for user:",
        user.displayName,
        "role:",
        user.role,
      );
    ...
    async (scannedId: string) => {
      ...
      try {
        ...
        if (!user) {
          ...
          setError(
            `Badge ${scannedId} not found in system. Please contact administrator.`,
          );
          setIsLoading(false);
          return;
        }
        ...
        console.log(
          "[LoginScreen] Badge login successful for user:",
          user.displayName,
          "role:",
          user.role,
          "badgeId:",
  ...
  const handleUsernameChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const isRapidInput =
        timeSinceLastInput < 100 &&
        value.length > inputBufferRef.current.length;
          ...
          if (badgeId) {
            ...
            if (user) {
              ...
              login({
                ...
                .then(() => {
                  console.log(
                    "[LoginScreen] Keyboard wedge login successful for user:",
                    user.displayName,
                    "role:",
                    user.role,
                  );
                ...
                .catch((err) => {
                  console.error(
                    "[LoginScreen] Keyboard wedge login failed:",
                    err,
                  );
                  setError(
                    "Network error. Please check your connection and try again.",
    ...
    >
      {error && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-md px-4">
          <Alert
            variant="destructive"
            className="bg-red-900/95 border-red-500 shadow-2xl"
          >
