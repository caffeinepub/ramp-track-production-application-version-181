...
import { validateBadgeScan, validateCredentials } from "../data/userRoster";
...
const defaultAuthContext: AuthContextType = {
  ...
  loginError: null,
  login: async () => {
    console.error("[AuthContext] login called before provider mounted");
  },
  logout: () => {},
...
export function AuthProvider({ children }: { children: ReactNode }) {
  console.log(
    "[AuthContext] Provider mounting, instance:",
    AUTH_CONTEXT_INSTANCE_ID,
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (
        storedAuthJson === null ||
        storedAuthJson === "undefined" ||
        storedAuthJson === ""
      ) {
        console.log(
          '[AUTH] No valid stored session found (null, "undefined", or empty)',
        );
        ...
        try {
          ...
          if (storedData?.username) {
            console.log(
              "[AUTH] Successfully parsed from ramptrack_v2_session:",
              storedData,
            );
            ...
            const authData: AuthState = {
              ...
              role:
                storedData.roles && Array.isArray(storedData.roles)
                  ? storedData.roles[0]
                  : "guest",
              badgeId: storedData.badgeId || storedData.username,
              name:
                storedData.displayName || storedData.username || "Signed out",
            };

            console.log(
              "[AUTH] Constructed authData from ramptrack_v2_session:",
              authData,
            );
            ...
            console.warn(
              "[AUTH] ramptrack_v2_session exists but invalid structure, removing",
            );
            try {
          ...
          console.error(
            "[AUTH] Failed to parse localStorage data:",
            parseError,
          );
      ...
      console.log(
        "[AUTH] Hydration completed - hydrationCompleted set to true",
      );
  ...
  useEffect(() => {
    if (hydrationCompleted) {
      console.log(
        "[AUTH] IsAuthReady triggered by hydrationCompleted:",
        hydrationCompleted,
      );
  ...
  const setAuth = useCallback((authData: AuthState) => {
    ...
    console.log(
      "[AUTH WRITE] ramptrack_v2_session readback=",
  ...
  const login = useCallback(
    async (credentials: {
      ...
      try {
        ...
        let user:
          | ReturnType<typeof validateBadgeScan>
        ...
        console.log(
          "[AUTH] Local validation successful, setting auth immediately",
        );
        ...
        console.log(
          "[AUTH] AUTH_LOGIN_OK - success for:",
          user.displayName,
          "role:",
          user.role,
        );
