...
export async function clearCachedApp(): Promise<void> {
  ...
  try {
    ...
    if ("serviceWorker" in navigator) {
      ...
      console.log(
        `[clearCachedApp] Found ${registrations.length} service worker(s)`,
      );

      for (const registration of registrations) {
        await registration.unregister();
        console.log(
          "[clearCachedApp] Unregistered service worker:",
          registration.scope,
        );
      }
    }
    ...
    if ("caches" in window) {
      ...
      console.log(
        `[clearCachedApp] Found ${cacheNames.length} cache(s):`,
        cacheNames,
      );
    ...
    alert(
      "Error clearing cache. Please try manually clearing your browser cache.",
    );
  }
}
