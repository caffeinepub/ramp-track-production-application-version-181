      ...
      if (source === "currentUser") {
        ...
        if (
          ...
          return {
            ...
            displayName:
              rawData.displayName || rawData.username || "Signed out",
            role: rawData.roles[0] || "guest",
          };
        }
      } else {
        // Canonical format: { user, role, badgeId?, name?, ts? }
        if (rawData?.user && rawData.role) {
          return {
            employeeId: rawData.badgeId || rawData.user,
            displayName:
              rawData.name || rawData.badgeId || rawData.user || "Signed out",
            role: rawData.role,
          };
        }
      }
...
export async function ensureUserContext(operatorId?: string): Promise<boolean> {
  try {
    ...
    console.log(
      "[ensureUserContext] Session valid for:",
      localContext.displayName,
    );
    return true;
  } catch (error: any) {
    ...
    if (isAuthFailure) {
      console.error(
        "[ensureUserContext] Authentication failure:",
        errorMessage,
      );
      return false;
    }
    ...
    console.warn(
      "[ensureUserContext] Backend validation failed (non-fatal):",
      errorMessage,
    );
    return true;
  }
